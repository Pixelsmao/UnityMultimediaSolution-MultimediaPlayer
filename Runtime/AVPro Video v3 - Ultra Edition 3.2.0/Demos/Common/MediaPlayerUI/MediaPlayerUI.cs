...
namespace RenderHeads.Media.AVProVideo.Demos
{
    ...
    {
        [SerializeField] public MediaPlayer _mediaPlayer = null;

        [Header("Options")] [SerializeField] public float _keyVolumeDelta = 0.05f;
        [SerializeField] public float _jumpDeltaTime = 5f;
        [SerializeField] public bool _showOptions = true;
        [SerializeField] public bool _showDebug = false;
        [SerializeField] public bool _autoHide = true;
        [SerializeField] public float _userInactiveDuration = 1.5f;
        [SerializeField] public bool _useAudioFading = true;

        [Header("Keyboard Controls")] [SerializeField]
        public bool _enableKeyboardControls = true;

        [SerializeField] public KeyCode KeyVolumeUp = KeyCode.UpArrow;
        [SerializeField] public KeyCode KeyVolumeDown = KeyCode.DownArrow;
        [SerializeField] public KeyCode KeyTogglePlayPause = KeyCode.Space;
        [SerializeField] public KeyCode KeyToggleMute = KeyCode.M;
        [SerializeField] public KeyCode KeyJumpForward = KeyCode.RightArrow;
        [SerializeField] public KeyCode KeyJumpBack = KeyCode.LeftArrow;

        [Header("Optional Components")] [SerializeField]
        OverlayManager _overlayManager = null;

        [SerializeField] MediaPlayer _thumbnailMediaPlayer = null;
        [SerializeField] RectTransform _timelineTip = null;

        [Header("UI Components")] [SerializeField]
        RectTransform _canvasTransform = null;
        ...
        [Header("UI Components (Optional)")] [SerializeField]
        GameObject _liveItem = null;
        ...
        private void MuteAudio(bool mute)
        {
            if (_mediaPlayer && _mediaPlayer.Control != null)
            {
                ...
                if (_overlayManager)
                {
                    _overlayManager.TriggerFeedback(mute
                        ? OverlayManager.Feedback.VolumeMute
                        : OverlayManager.Feedback.VolumeUp);
                }
            }
        }
            ...
            if (_mediaPlayer && _mediaPlayer.Control != null)
            {
                ...
                if (audioSource && _audioSpectrumMaterial)
                {
                    ...
                    for (int i = 0; i < sampleRange; i++)
                    {
                        ...
                        _spectrumSamplesSmooth[i] = Mathf.Lerp(_spectrumSamplesSmooth[i], newSample,
        ...
        void Update()
        {
            ...
            if (_mediaPlayer.Info != null)
            {
                ...
                if (_timelineTip != null)
                {
                    ...
                    {
                        Vector2 canvasPos;
                        RectTransformUtility.ScreenPointToLocalPointInRectangle(_canvasTransform, Input.mousePosition,
                            null, out canvasPos);
                        ...
                        if (UserInteraction.IsUserInputThisFrame())
                        {
...

                            {
                                ...
                                if (_segmentsSeek != null)
                                {
                                    ...
                                    {
                                        double t = ((_mediaPlayer.Control.GetCurrentTime() - timelineRange.startTime) /
                ...
                {
                    string t1 = Helper.GetTimeString((_mediaPlayer.Control.GetCurrentTime() - timelineRange.startTime),
                        false);
                ...
                if (_sliderTime && !_isHoveringOverTimeline)
                {
                    ...
                    if (timelineRange.duration > 0.0)
                    {
                        t = ((_mediaPlayer.Control.GetCurrentTime() - timelineRange.startTime) /
                             timelineRange.duration);
                    }
...
//						resolutionName = Helper.GetFriendlyResolutionName(_mediaPlayer.Info.GetVideoWidth(), _mediaPlayer.Info.GetVideoHeight(), _mediaPlayer.Info.GetVideoFrameRate());
                        ...
                        {
                            resolutionName = string.Format("{0} x {1} @ {2}", _mediaPlayer.Info.GetVideoWidth(),
                                _mediaPlayer.Info.GetVideoHeight(), fps.ToString("0.00"));
                        }
                        else
                        {
                            resolutionName = string.Format("{0} x {1}", _mediaPlayer.Info.GetVideoWidth(),
                                _mediaPlayer.Info.GetVideoHeight());
                        }
                    }
                ...
                if (_segmentsBuffered)
                {
                    ...
                    if (times.Count > 0 && timelineRange.duration > 0.0)
                    {
                        ...
                        {
                            ranges[i * 2 + 0] = Mathf.Max(0f,
                                (float)((times[i].StartTime - timelineRange.startTime) / timelineRange.duration));
                            ranges[i * 2 + 1] = Mathf.Min(1f,
                                (float)((times[i].EndTime - timelineRange.startTime) / timelineRange.duration));
                        }
                    }
                ...
                if (_segmentsProgress)
                {
                    ...
                    if (times.Count > 0 && timelineRange.Duration > 0.0)
                    {
                        ...
                        double x2 = ((_mediaPlayer.Control.GetCurrentTime() - timelineRange.startTime) /
                                     timelineRange.duration);
        ...
        void OnGUI()
        {
            ...
            GUILayout.Label(string.Format("Duration: {0}s\tFPS: {1}", _mediaPlayer.Info.GetDuration(),
                _mediaPlayer.Info.GetVideoDisplayRate().ToString("F2")));
